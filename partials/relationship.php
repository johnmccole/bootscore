...
<section <?php if($section_id): echo 'id="'.$section_id.'"'; endif; ?>  class="relationship-section">
  <div class="<?= $container; ?>">
    <?php if($isotope): ?>
    <?php get_template_part('partials/category-filters-sort'); ?>
    <?php endif; ?>

  	<div class="relationship <?php if($isotope): echo 'grid'; endif; ?> d-flex flex-wrap justify-content-evenly <?= $classes ?>" <?php if($slick && $isotope == 0): echo 'data-id="slick-relationship"'; endif; ?>>
    <?php if($selected_posts): foreach($selected_posts as $post): setup_postdata($post); ?>
      <?php $terms = get_the_category($post->ID); ?>
    	<div data-id="<?= $post->ID ?>" class="card me-3 <?php foreach($terms as $term): echo ' ' . $term->slug; endforeach; ?>" style="width: 18rem;">
		  <img src="<?= the_post_thumbnail_url() ?>" class="card-img-top" alt="<?= the_title() ?> Featured Image">
		  <div class="card-body">
		    <h5 class="card-title name"><?= the_title() ?></h5>
